...
function Profile() {
  // Gets logged-in user data from localStorage
  const storedUser = localStorage.getItem("user");

  // Stores the customer's matching orders
  const [orders, setOrders] = useState([]);

  // Tracks loading state while fetching orders
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:8000/api/orders/")
      ...
      .then((data) => {
        // Only keeps orders matching the logged-in user's email
